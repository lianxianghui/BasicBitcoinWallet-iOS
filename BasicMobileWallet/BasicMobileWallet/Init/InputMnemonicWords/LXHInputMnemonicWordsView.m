...
@interface LXHInputMnemonicWordsView() <UITableViewDataSource, UITableViewDelegate>
@end

@implementation LXHInputMnemonicWordsView
...
- (void)addSubviews {
    ...
    [self addSubview:self.inputTextFieldWithPlaceHolder];
...
- (void)makeConstraints {
    [self.listView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.equalTo(self.inputTextFieldWithPlaceHolder.mas_bottom).offset(12);
    }];
    [self.inputTextFieldWithPlaceHolder mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.mas_left).offset(20);
        make.right.equalTo(self.mas_right).offset(-20);
        make.top.equalTo(self.promot.mas_bottom).offset(10);
    ...
    [self.promot mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.right.equalTo(self.inputTextFieldWithPlaceHolder.mas_right).offset(-19);
    }];
}
...
- (UILabel *)title {
    if (!_title) {
        ...
        [textAttributes setObject:paragraphStyle forKey:NSParagraphStyleAttributeName];
        NSAttributedString *text = [[NSAttributedString alloc] initWithString:NSLocalizedString(@"输入助记词", nil) attributes:textAttributes];
...
- (UILabel *)promot {
    if (!_promot) {
        ...
        [textAttributes setObject:paragraphStyle forKey:NSParagraphStyleAttributeName];
        text = [[NSAttributedString alloc] initWithString:NSLocalizedString(@"请按顺序逐个输入助记词", nil) attributes:textAttributes];

...
- (UILabel *)promot {
    if (!_promot) {
        ...
        NSAttributedString *text = [[NSAttributedString alloc] initWithString:NSLocalizedString(@"由于您正在恢复已有的钱包，这需要联网搜索有哪些地址已经被使用过，这样才能保证获取已有交易信息。这是恢复钱包的最后一步，点击搜索并生成按钮开始恢复。\n(如果您打算把当前正在恢复的钱包作为冷钱包使用，请点击仅生成按钮）", nil) attributes:textAttributes];

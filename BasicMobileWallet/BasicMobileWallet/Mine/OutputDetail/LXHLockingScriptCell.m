...
@interface LXHLockingScriptCell()
@end

@implementation LXHLockingScriptCell
...
- (UILabel *)title {
    if (!_title) {
        ...
        [textAttributes setObject:@(-0.3376471) forKey:NSKernAttributeName];
        [textAttributes setObject:paragraphStyle forKey:NSParagraphStyleAttributeName];
        NSAttributedString *text = [[NSAttributedString alloc] initWithString:NSLocalizedString(@"锁定脚本", nil) attributes:textAttributes];

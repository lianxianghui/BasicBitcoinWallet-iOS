...
@interface LXHTextRightIconCell()
@end

@implementation LXHTextRightIconCell
...
- (UILabel *)text {
    if (!_text) {
        ...
        NSAttributedString *text = [[NSAttributedString alloc] initWithString:NSLocalizedString(@"交易列表", nil) attributes:textAttributes];
...
- (UIImageView *)disclosureIndicator {
    if (!_disclosureIndicator) {
        ...
        _disclosureIndicator.image = [UIImage imageNamed:@"mine_mine_disclosure_indicator"];
    }

...
@interface LXHForgotPINAfterWalletDataGeneratedView()
@end

@implementation LXHForgotPINAfterWalletDataGeneratedView
...
- (void)makeConstraints {
    ...
    [self.button1 mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.mas_left).offset(19);
        make.right.equalTo(self.mas_right).offset(-19);
        make.bottom.equalTo(self.button2.mas_top).offset(-11);
        make.height.mas_equalTo(46);
    }];
...
- (UIButton *)button2 {
    if (!_button2) {
        ...
        NSAttributedString *text = [[NSAttributedString alloc] initWithString:NSLocalizedString(@"输入钱包助记词", nil) attributes:textAttributes];
...
- (UIButton *)button1 {
    if (!_button1) {
        ...
        [textAttributes setObject:@(-0.449999988079071) forKey:NSKernAttributeName];
        [textAttributes setObject:paragraphStyle forKey:NSParagraphStyleAttributeName];
        NSAttributedString *text = [[NSAttributedString alloc] initWithString:NSLocalizedString(@"扫描扩展公钥", nil) attributes:textAttributes];
...
- (UILabel *)desc1 {
    if (!_desc1) {
        ...
        [textAttributes setObject:paragraphStyle forKey:NSParagraphStyleAttributeName];
        text = [[NSAttributedString alloc] initWithString:NSLocalizedString(@"PIN码无法找回，只能重置。为了防止您的钱包被盗用，在重置之前需要确定您有权使用该钱包。请您根据之前生成钱包时选择的类型，在以下两种情况中选择一种进行处理：\n1.只读钱包：\n", nil) attributes:textAttributes];
        ...
        [textAttributes setObject:@(-0.8014479) forKey:NSKernAttributeName];
        [textAttributes setObject:paragraphStyle forKey:NSParagraphStyleAttributeName];
        text = [[NSAttributedString alloc] initWithString:NSLocalizedString(@" https://github.com/lianxianghui/BasicMobileWallet-iOS", nil) attributes:textAttributes];
        [attributedText appendAttributedString:text];
...
- (UILabel *)title {
    if (!_title) {
        ...
        [textAttributes setObject:paragraphStyle forKey:NSParagraphStyleAttributeName];
        NSAttributedString *text = [[NSAttributedString alloc] initWithString:NSLocalizedString(@"忘记PIN码的处理方法", nil) attributes:textAttributes];

...
@interface LXHBuildTransactionView()
@end

@implementation LXHBuildTransactionView
...
- (UILabel *)title {
    if (!_title) {
        ...
        NSAttributedString *text = [[NSAttributedString alloc] initWithString:NSLocalizedString(@"构建交易", nil) attributes:textAttributes];
...
- (UIButton *)rightTextButton {
    if (!_rightTextButton) {
        _rightTextButton = [UIButton buttonWithType:UIButtonTypeSystem];//Has System Highlighted color
        _rightTextButton.backgroundColor = UIColorFromRGBA(0xFFFFFF00);
        _rightTextButton.layer.cornerRadius = 2;
        _rightTextButton.alpha = 1;
        _rightTextButton.contentHorizontalAlignment = UIControlContentHorizontalAlignmentLeft;
        ...
        [textAttributes setObject:UIColorFromRGBA(0x4A90E2FF) forKey:NSForegroundColorAttributeName];
        [textAttributes setObject:font forKey:NSFontAttributeName];
        [textAttributes setObject:@(-0.4013889) forKey:NSKernAttributeName];
        [textAttributes setObject:paragraphStyle forKey:NSParagraphStyleAttributeName];
        text = [[NSAttributedString alloc] initWithString:NSLocalizedString(@"下一步", nil) attributes:textAttributes];
        [attributedText appendAttributedString:text];

        [_rightTextButton setAttributedTitle:attributedText forState:UIControlStateNormal];
        _rightTextButton.contentHorizontalAlignment = UIControlContentHorizontalAlignmentLeft;
    }

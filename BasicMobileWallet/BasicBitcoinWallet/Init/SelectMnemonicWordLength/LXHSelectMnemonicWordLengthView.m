// LXHSelectMnemonicWordLengthView.m
// BasicWallet
//
...
- (void)makeConstraints {
    [self.listView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.right.equalTo(self.customNavigationBar.mas_right).offset(-20);
...
- (UILabel *)title {
    if (!_title) {
        ...
        NSAttributedString *text = [[NSAttributedString alloc] initWithString:NSLocalizedString(@"助记词数量", nil) attributes:textAttributes];
...
- (UILabel *)promot {
    if (!_promot) {
        ...
        [textAttributes setObject:paragraphStyle forKey:NSParagraphStyleAttributeName];
        text = [[NSAttributedString alloc] initWithString:NSLocalizedString(@"请选择助记词的数量", nil) attributes:textAttributes];
